...
public final class StationPartition implements StationConnectivity{
	...
	public boolean connected(Station s1, Station s2) {
		if(s1.id() < stationsConnections.length && s2.id() < stationsConnections.length && stationsConnections[s1.id()] == stationsConnections[s2.id()]) {
		    return true;
		}
		else if(s1.id() == s2.id()) {
			return true;
	...
	public static final class Builder{
		 ...
		 * The integer in the array represents the "reference station" of the disjoint set to which the index station belongs.
		 */
		private int[] stationsConnections;
